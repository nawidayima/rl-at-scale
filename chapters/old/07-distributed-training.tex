...
This is synchronous and latency-sensitive --- on the critical path.
...
\begin{tikzpicture}[
  ...
  gen/.style={phase, fill=beni!15},
  rew/.style={phase, fill=kitsune!15},
  sync/.style={phase, fill=sumi!25, pattern=north east lines, pattern color=sumi!40},
  train/.style={phase, fill=aiiro!15}
]
  % GPU labels
  \node[font=\footnotesize\bfseries, anchor=east] at (-3, 3) {GPU 0};
  \node[font=\footnotesize\bfseries, anchor=east] at (-3, 2) {GPU 1};
  \node[font=\footnotesize\bfseries, anchor=east] at (-3, 1) {GPU 2};
  \node[font=\footnotesize\bfseries, anchor=east] at (-3, 0) {GPU 3};
  ...
  % Sync barrier
  \draw[dashed, thick, sumi!50] (80, -0.5) -- (80, 3.5);
  \node[font=\scriptsize\itshape, sumi!60, rotate=90, anchor=south] at (81, 1.5) {sync barrier};

  % Time axis
  \draw[-{Stealth[length=4pt]}, sumi!40] (0, -0.7) -- (100, -0.7)
    node[right, font=\scriptsize] {time};

  % Legend
  \node[font=\scriptsize] at (50, -1.5) {%
    \tikz{\fill[beni!15] (0,0) rectangle (0.3,0.2);} generation \quad
    \tikz{\fill[kitsune!15] (0,0) rectangle (0.3,0.2);} reward \quad
    \tikz{\fill[sumi!25, pattern=north east lines, pattern color=sumi!40] (0,0) rectangle (0.3,0.2);} idle (bubble) \quad
    \tikz{\fill[aiiro!15] (0,0) rectangle (0.3,0.2);} training};
\end{tikzpicture}
\end{center}
...
Estimate first: for 8 GPUs exchanging 1 GB over 100 GB/s links, how long does a ring all-reduce take? Then check below.
...
\begin{tikzpicture}[
  ...
  % 4 GPUs in a ring
  \node[gpu=aiiro!10]   (g0) at (0, 1.5)  {GPU 0};
  \node[gpu=matcha!10]  (g1) at (2.5, 0)  {GPU 1};
  \node[gpu=kitsune!10] (g2) at (0, -1.5) {GPU 2};
  \node[gpu=beni!10]    (g3) at (-2.5, 0) {GPU 3};

  % Ring arrows (clockwise)
  \draw[arr=aiiro!50]   (g0) -- node[right, font=\scriptsize] {$S/N$} (g1);
  \draw[arr=matcha!50]  (g1) -- node[right, font=\scriptsize] {$S/N$} (g2);
  \draw[arr=kitsune!50] (g2) -- node[left, font=\scriptsize]  {$S/N$} (g3);
  \draw[arr=beni!50]    (g3) -- node[left, font=\scriptsize]  {$S/N$} (g0);

  % Phase labels
  \node[font=\footnotesize\itshape, sumi!60] at (4.2, 1.5) {Phase 1: reduce-scatter};
  \node[font=\footnotesize\itshape, sumi!60] at (4.2, 0.8) {($N{-}1$ steps)};
  \node[font=\footnotesize\itshape, sumi!60] at (4.2, -0.5) {Phase 2: all-gather};
  \node[font=\footnotesize\itshape, sumi!60] at (4.2, -1.2) {($N{-}1$ steps)};
