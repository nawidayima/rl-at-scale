...
TinyRL is the MNIST of RLVR: a shared substrate where ideas can be tested cheaply, compared cleanly, and built upon publicly. The headline environment, TinyRL-Code, is RLVR in miniature --- verifiable rewards, deterministic unit tests, emergent strategies, and a reward hacking gallery --- all on a single GPU in under an hour.
...
The TinyStories paper (\href{https://arxiv.org/abs/2305.07759}{arXiv:2305.07759}) showed that language model phenomena --- memorization, generalization, induction heads, grokking --- are observable at toy scale if the environment is designed carefully. Four properties made it work:
...
\begin{tikzpicture}[
  ...
  % Era labels
  \node[lbl] at (0.8, 2) {2020--22};
  \node[lbl] at (0.8, 1) {2022--24};
  \node[lbl] at (0.8, 0) {2025+};

  % 2020-22: pretraining only
  \node[era, fill=aiiro!15, minimum width=8.5cm, anchor=west] at (1, 2) {Pretraining};

  % 2022-24: pretraining + SFT + RLHF
  \node[era, fill=aiiro!15, minimum width=6.5cm, anchor=west] at (1, 1) {Pretraining (95\%)};
  \node[era, fill=sumi!15, minimum width=0.7cm, anchor=west] at (7.65, 1) {\tiny SFT};
  \node[era, fill=kitsune!15, minimum width=0.9cm, anchor=west] at (8.5, 1) {\tiny RLHF};

  % 2025+: pretraining + SFT + RLVR + RLHF
  \node[era, fill=aiiro!12, minimum width=4.2cm, anchor=west] at (1, 0) {Pretraining};
  \node[era, fill=sumi!12, minimum width=0.7cm, anchor=west] at (5.35, 0) {\tiny SFT};
  \node[era, fill=beni!20, minimum width=2.5cm, anchor=west] at (6.2, 0) {RLVR};
  \node[era, fill=kitsune!12, minimum width=0.7cm, anchor=west] at (8.85, 0) {\tiny RLHF};

  % Arrow showing shift
  \draw[-{Stealth[length=5pt]}, thick, beni!50] (7.45, 0.5) -- (7.45, 0.85)
    node[right, font=\scriptsize\itshape, beni!60] {growing};
\end{tikzpicture}
\end{center}
