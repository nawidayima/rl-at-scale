...
Ranges are regime-dependent --- they shift with model size, sequence length, and hardware. The point is the ordering, not the exact numbers. Rollout generation is 60--80\% of wall-clock --- this is why Chapter~3 is the centerpiece of this book. Cutting rollout count in half saves 30--40\% of total training cost. Cutting optimizer steps in half saves 3--8\%.
...
The first three are ordered by steady-state cost impact. The fourth prevents catastrophic waste rather than improving per-step efficiency --- a single early intervention can save more compute than all other levers combined.
...
Early in training, $\beta$ barely constrains --- the reference forward pass (5--10\% of step time per the cost breakdown) produces no useful signal. Late in training, the same $\beta$ may be too loose, causing collapse-and-recovery episodes that consume hundreds of steps.

\textbf{The freshness connection.} KL penalty directly determines how quickly your rollouts go stale. Tight KL (large $\beta$) means the policy moves slowly --- rollouts stay on-policy longer, relaxing Chapter~3's freshness constraint. You can get away with a larger actor/learner ratio and more reuse of generated rollouts. Loose KL (small $\beta$) means the policy changes fast --- you need fresher rollouts, more generation throughput, and tighter synchronization between the actor and learner from Chapter~3. The algorithmic choice of $\beta$ has a direct systems consequence for how you architect the generation pipeline.
...
The four levers above are not things to memorize --- they are things to test. The core skill this chapter teaches is ablation: systematically isolating whether a design choice is load-bearing by removing or varying it and measuring the impact.
