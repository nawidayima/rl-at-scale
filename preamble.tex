% rl-at-scale shared preamble
% Packages and custom environments used across all documents

% --- Packages ---
\usepackage{amsmath, amssymb}
\usepackage{enumitem}
\usepackage{titlesec}
\usepackage[hidelinks]{hyperref}
\usepackage{fontenc}
\usepackage{lmodern}
\usepackage{booktabs}
\usepackage{xcolor}
% --- Nippon Seasons palette (日本の伝統色) ---
\definecolor{kitsune}{HTML}{C3803A}   % 狐色 fox, autumn
\definecolor{aiiro}{HTML}{004C71}     % 藍色 indigo, winter
\definecolor{beni}{HTML}{B14A4A}      % 紅 crimson, summer
\definecolor{matcha}{HTML}{7B8B4E}    % 抹茶 tea green, spring
\definecolor{sumi}{HTML}{4A4A4A}      % 墨 ink
% Colorblind-safe code syntax (Okabe-Ito derived)
\definecolor{codekey}{HTML}{0077BB}
\definecolor{codecomm}{HTML}{7F7F7F}
\definecolor{codestr}{HTML}{009988}
\usepackage{tcolorbox}
\tcbuselibrary{breakable}
\usepackage{listings}
\usepackage{fancyvrb}
\usepackage{tikz}
\usetikzlibrary{positioning, arrows.meta, calc, patterns, fit, decorations.pathreplacing}

% --- Workbook boxes ---

% Exercise box: kitsune
\newtcolorbox{exercise}[1][]{
  colback=kitsune!5!white, colframe=kitsune!80!black, boxrule=0.6pt,
  left=5pt, right=5pt, top=3pt, bottom=3pt,
  fonttitle=\bfseries\small, title=Exercise: #1,
  breakable,
  phantom={\addcontentsline{toc}{subsection}{\texorpdfstring{\textit{Exercise: #1}}{Exercise: #1}}}
}

% Think-first box: aiiro
\newtcolorbox{thinkfirst}[1][]{
  colback=aiiro!5!white, colframe=aiiro!70!black, boxrule=0.5pt,
  left=5pt, right=5pt, top=3pt, bottom=3pt,
  fonttitle=\bfseries\small, title={Think First (no computer): #1},
  breakable,
  phantom={\addcontentsline{toc}{subsection}{\texorpdfstring{\textit{Think First: #1}}{Think First: #1}}}
}

% Portfolio project box: beni
\newtcolorbox{portfolio}[1][]{
  colback=beni!5!white, colframe=beni!80!black, boxrule=0.6pt,
  left=5pt, right=5pt, top=3pt, bottom=3pt,
  fonttitle=\bfseries\small, title={Portfolio Project: #1},
  breakable,
  phantom={\addcontentsline{toc}{subsection}{\texorpdfstring{\textbf{Portfolio: #1}}{Portfolio: #1}}}
}

% Why-this-matters box: matcha
\newtcolorbox{whythis}{
  colback=matcha!5!white, colframe=matcha!70!black, boxrule=0.5pt,
  left=5pt, right=5pt, top=3pt, bottom=3pt,
  fonttitle=\bfseries\small, title=Why This Matters at Frontier Scale
}

% Research taste box: sumi
\newtcolorbox{taste}{
  colback=sumi!5!white, colframe=sumi!70!black, boxrule=0.5pt,
  left=5pt, right=5pt, top=3pt, bottom=3pt,
  fonttitle=\bfseries\small, title=Research Taste Check
}
